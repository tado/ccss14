...
Stats::Stats():top(0), left(0), width(80), height(48), maxValue(120), lapTime(100), mode(0){
    lastLap = 0;
    lastMs = 0;
    padding = 3;
}
...
void Stats::mousePressed(int x, int y, int button){
...
void Stats::setPosition(int _left, int _top){
    left = _left;
    top = _top;
}

void Stats::setSize(int _width, int _height){
    width = _width;
    height = _height;
}

void Stats::setMaxValue(float _maxValue){
    maxValue = _maxValue;
}

void Stats::setLapTime(float _lapTime){
    lapTime = _lapTime;
}